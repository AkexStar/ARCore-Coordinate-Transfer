 ...
 */
package com.google.ar.core.ARPositioning.java.common.helpers;
...
public class DepthSettings {
  ...
  public void setUseDepthForOcclusion(boolean enable) {
    if (enable == useDepthForOcclusion) {
      return; // No change. 如果设置没有变化回退即可
    }

    // Updates the stored default settings. 设置变化了就使用enable来更新
   ...
   * 确定是否应显示使用基于深度的遮挡的初始提示。*/
  public boolean shouldShowDepthEnableDialog() {
    // Checks if this dialog has been called before on this device. 检查之前是否在此设备上调用过此对话框。
    boolean showDialog =
    ...
    if (showDialog) {
      ...
      // 只在第一次显示对话框。如果用户想要再次调整这些设置，他们可以使用齿轮图标来调用设置菜单对话框。
